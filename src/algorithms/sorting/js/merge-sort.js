const mergeSort = function (array) {
  ...
  const middle = Math.floor(array.length / 2); // 중간 값
  // slice()로 새로운 배열 생성 - 원본 영향 no
  const left = array.slice(0, middle); // 0 ~ middle(middle 미포함) 
  const right = array.slice(middle, array.length); // middle ~ 마지막 요소
