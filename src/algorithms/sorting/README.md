...
+ 힙 구성은 최악의 경우 삽입 삭제 연산 모두 힙 트리의 높이 만큼 이동해야 하므로, 빅오 표기법으론 O(logn)이 소요된다. 크기가 n인 데이터로 힙 트리를 구성 한다고 생각 해봤을 때, n번의 삽입과 각 삽입 시 최대 O(logn)만큼의 이동이 발생하게 되어 O(nlogn)의 복잡도가 소요 된다. </br>
