...
class MaxHeap {
  ...
  printArray() {
    for (let i = 1; i < this.maxheap.length; i++) process.stdout.write(`${this.maxheap[i]} `);
    console.log("");
  }
  ...
  insert(node) {
    ...
    while (current > 1) {
      // 부모 노드가 루트 노드 일 때까지
      if (
        this.maxheap[current] > this.maxheap[this.parentIndex(current)] &&
        this.parentIndex(current) >= 1
      ) {
